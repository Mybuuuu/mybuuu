...
const RibbonSegment = ({ 
  ...
  switch (type) {
    case 'title':
      return (
        <div className="flex flex-col items-center justify-center py-8 px-2 w-full flex-shrink-0">
          <div className="h-[200px] flex items-center justify-center">
            <span className={`rotate-90 origin-center whitespace-nowrap tracking-[0.25em] font-space font-extrabold text-2xl uppercase ${getTextColorClass()}`}>
              {text}
            </span>
          </div>
        </div>
    ...
    case 'detail':
      return (
        <div className="flex flex-col items-center justify-center py-6 px-1 w-full flex-shrink-0">
          <div className="h-[260px] flex items-center justify-center">
            <span className={`rotate-90 origin-center whitespace-nowrap tracking-[0.2em] font-sans font-bold text-xs uppercase ${
              colorTheme === 'black' 
                ? 'text-black/80 font-extrabold' 
                : 'text-transparent [-webkit-text-stroke:0.5px_rgba(255,255,255,0.4)] opacity-70'
            }`}>
              {text}
            </span>
          </div>
    ...
    case 'symbol':
      return (
        <div className="flex flex-col items-center justify-center py-4 w-full flex-shrink-0">
          {text === '★' && (
            <svg className={`w-6 h-6 ${getTextColorClass()}`} viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" />
            </svg>
          )}
          {text === '◆' && (
            <div className={`w-5 h-5 rotate-45 border-2 ${
    ...
    case 'divider':
      return (
        <div className="w-full h-8 flex-shrink-0 overflow-hidden relative opacity-60">
          <div 
            className="absolute inset-0" 
            style={{
              backgroundImage: colorTheme === 'gold'
                ? 'repeating-linear-gradient(-45deg, var(--accent-gold) 0, var(--accent-gold) 6px, transparent 6px, transparent 12px)'
                : 'repeating-linear-gradient(-45deg, var(--accent-secondary) 0, var(--accent-secondary) 6px, transparent 6px, transparent 12px)',
            }}
          />
        </div>
      );
    ...
    case 'barcode':
      return (
        <div className="flex flex-col items-center justify-center py-6 w-full flex-shrink-0">
          <div className="flex items-center justify-center gap-[2px] h-10 w-12 opacity-60">
          ...
          <span className={`text-[8px] font-mono tracking-widest mt-1 opacity-50 ${colorTheme === 'black' ? 'text-black' : 'text-white'}`}>
            {text || 'ID-03.06.26'}
          </span>
        </div>
      );
  ...
  return (
    <motion.div
      ...
      className={`fixed top-0 bottom-0 select-none pointer-events-none shadow-[0_15px_35px_rgba(0,0,0,0.65)] ribbon-texture ribbon-shading ${
        strip.stitchClass
      } ${strip.bgClass} ${strip.hiddenOnMobile ? 'hidden md:flex' : 'flex'} flex-col`}
    >
